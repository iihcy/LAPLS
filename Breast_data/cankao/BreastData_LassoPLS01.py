...
#Lasso函数
def lasso_regression(X, y, lambd, threshold=0.135):
    '''
...
def lasso_traj(e0, f0, ntest):
    ...
    for i in range(ntest):
        w = lasso_regression(e0, f0, lambd=exp(i-12))
        ws[i, :] = w.T
...
if __name__ == '__main__':
    x0, y0 = loadDataSet01('BreastData01.txt')#单因变量与多因变量
    # 随机划分数据集- 7:3
    ...
    # dataframe.to_csv(u'回归系数迭代002.csv')
    ...
    dataframe.to_csv(u'BreastDataTrain01xx.csv')
    dataframe = pd.DataFrame(train_y)
    dataframe.to_csv(u'BreastDataTrain01yy.csv')
    count = 0
    for i in range(m):
        if xish[i] == 0:
            count += 1
